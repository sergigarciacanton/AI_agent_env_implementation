...
class EnvironmentUPC(gym.Env):
    ...
    def send_action_to_fec(self, action, fec_id):
        host = self.fec_dict[str(fec_id)]['ip']
    ...
    def move_background_vehicles(self):
        ...
        for vehicle_id, vehicle in self.background_vehicles.items():
            ...
            if vehicle.my_fec != requesting_fec_for_vehicle:
                ...
                if self.vecn[str(requesting_fec_for_vehicle)]['gpu'] >= vnf_gpu and \
                        self.vecn[str(requesting_fec_for_vehicle)]['ram'] >= vnf_ram and \
                        self.vecn[str(requesting_fec_for_vehicle)]['bw'] >= vnf_bw:

                    # Update FEC resources
                    self.used_fec_resources[str(requesting_fec_for_vehicle)][0] += vnf_gpu
                    self.used_fec_resources[str(requesting_fec_for_vehicle)][1] += vnf_ram
                    self.used_fec_resources[str(requesting_fec_for_vehicle)][2] += vnf_bw

                    # Only for not newly instantiated vehicles
                    if vehicle.my_fec != -1:
                        self.used_fec_resources[str(vehicle.my_fec)][0] -= vnf_gpu
                        self.used_fec_resources[str(vehicle.my_fec)][1] -= vnf_ram
                        self.used_fec_resources[str(vehicle.my_fec)][2] -= vnf_bw
    ...
    def reset(self, seed: int = None, nodes_to_evaluate=None) -> Tuple[Any, Dict[str, Any]]:
        ...
        # fec_id : (gpu, ram, bw)
        self.used_fec_resources = {str(i): [0, 0, 0] for i in FECS_RANGE.keys()}
    ...
    def step(self, action):
        ...
        vecn = [
            list(self.vecn[str(0)].values())[1:4],
            list(self.vecn[str(1)].values())[1:4],
            list(self.vecn[str(2)].values())[1:4],
            list(self.vecn[str(3)].values())[1:4]
        ]
